...
type FormData = {
  ...
  status: string;
  /** Which of this vendor's branches the machine stands in. */
  locationId: string;
};
...
export default function AddPrinterPage() {
  ...
  // A vendor may run several places; the printer has to be told which one it
  // stands in, so the same machine at two branches stays distinguishable.
  useEffect(() => {
    let alive = true;
    apiFetch<{ locations: Branch[] }>("/vendors/me/locations")
      .then((r) => {
        if (!alive) return;
        setBranches(r.locations);
        // With exactly one branch there is nothing to choose — pick it.
        if (r.locations.length === 1) set("locationId", r.locations[0].id);
      })
      .catch(() => {});
    return () => { alive = false; };
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/vendor/printers/${res.printer.id}?registered=1`);
    } catch (e: unknown) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      {/* Header */}
      <div className="mb-6">
        <Link href="/vendor/printers" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-slate-700 text-sm font-medium transition-colors">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {branches.length > 0 && (
              <div className="sm:col-span-2">
                ...
                <p className="mt-1.5 text-xs text-slate-500">
                  Which of your places this machine stands in. Manage your branches from the dashboard.
                </p>
              </div>
            )}
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center gap-3 pb-2 sticky bottom-0 bg-slate-50/80 backdrop-blur py-3 -mx-1 px-1 rounded-xl">
          <Link href="/vendor/printers" className="text-center text-sm font-semibold text-slate-500 hover:text-slate-800 transition-colors px-5 py-3 rounded-xl border border-slate-200 bg-white sm:border-0 sm:bg-transparent">
            Cancel
          </Link>
          <button
            type="submit"
