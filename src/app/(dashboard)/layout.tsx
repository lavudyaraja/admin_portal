...
import { apiFetch, getToken, clearToken, isConsoleRole, type AdminUser } from "@/lib/api";
import Sidebar from "@/components/Sidebar";
import SettingsModal, { type TabId } from "@/components/settings/SettingsModal";
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className={`${mainML} min-h-screen flex flex-col transition-[margin] duration-200`}>
        <header className="h-14 px-4 sm:px-6 flex items-center justify-between bg-white border-b border-slate-200 sticky top-0 z-30">
          ...
          <span className="text-[11px] font-bold text-slate-400 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100 uppercase tracking-wide">
            {user?.role}
          </span>
        </header>
